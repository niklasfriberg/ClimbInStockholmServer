				...
				for (int j = 0; j < 3; j++) {
					...
					if (xml.isRoute(i)) {
						if (xml.hasCoords(i)) {
							...
							hasRoutes = true;
							route.accumulate("Svårighet", xml.getGrade(xml.getName(i)));
							route.accumulate("RouteName", xml.getName(xml.getName(i)));
							route.accumulate("Beskrivning", xml.getDesc(i));
							route.accumulate("Höjd", xml.getHeigth(xml.getName(i)));
